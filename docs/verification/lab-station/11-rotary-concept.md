...
肘部兩段支撐已整合齒盤；目前保留 `support_envelope` 名稱以免被當成已通過製造契約的零件。接回段繞過齒盤與升降連桿。肩部／腕部已建立實體接頭；底座固定的製造與承力資格、腕部裝入路徑，以及肘部工具手柄／裝入掃掠與預緊保持力仍未取得資格。
...
使用者指出斷接後，移除獨立 support_envelope_2，前臂回接段與叉耳改為同一件；腕部設在四連桿後方 82 mm，透過一體橋接連到固定框。`wrist-interface.json` 在 −15／0／15° 檢查兩成員共同通孔、周邊實體與五金間隙；`red-wrist-disconnection.json` 移開叉耳必須拒絕。`shoulder-release.json` 使用同一五狀態齒槽判準。肩／腕證據不涵蓋底座；底座穿軸候選與其限制另列下節，整體承力仍未驗證。

## 底座穿軸候選

`lab_station_base` 為每個旋轉腳增加 OD12 mm 下伸軸頸、OD16／ID12.2 ×16 mm 名義金屬軸套，以及直達 6 mm 機箱底板的支承柱與側壁肋。名義 M5×90 穿軸由底面裝入，上方薄螺母與上下墊片保持；機箱增加 8 mm 底腳供螺栓頭避讓。五金未指定供應商、螺紋以間隙包絡表示，尚未驗證預緊、防鬆或工具裝入路徑。

`base-retention.json` 檢查共同通孔及周邊材料、五金對列印件中立間隙，以及每側五個位移後止擋接觸。`red-base-stop.json` 移走上墊片必須被拒絕。墊片鄰接處保留約 0.02 mm 幾何分離，避免共面三角形被當成穿透，並以位移後接觸檢查捕捉止擋；此數字不是列印配合公差或預緊量。

左側未確認泵浦佔位向機箱內側平移 10 mm，以避開支承柱。`base-section.png` 是切開機箱／上蓋／旋轉腳副本的診斷圖，原件未切割；剖面圖不可當列印件。既有 V10 機箱 STL 未包含這些改動，新機箱與旋轉腳仍需獨立網格／匯出契約。底板承力、任意姿態與線材均未取得資格。

支承柱頂端距上蓋底面 0.1 mm。上蓋離開名義接面 0.01 mm 後，機箱與上蓋不得有表面交叉，以區分盒緣的名義共面接觸；`red-base-lid.json` 增加穿入上蓋的凸柱必須被拒絕。這不是小於 0.01 mm 的穿透或製造公差資格。

使用者已要求減少機構與五金；新主線為 [12-simple-concept](12-simple-concept.md)。本頁四連桿／長穿軸候選保留作歷史與回歸，不再作定案方向。
