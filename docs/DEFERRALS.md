...
**狀態**：`due`（2026-09-08 記錄；同日觸發——手契約三條閘門首跑合計 8 s，遠低於 10 分鐘門檻）

**現況**：M1 把 **hand-v3 的兩份契約 + 重現差分**接進 `--real`（`scripts/ci.sh`），V1／V2／V6 的
七份契約仍是手動跑。觸發條件已成立；排入框架 M3（真機重驗那一片一起接，避免兩次量時間）。
