...
- 決定性與 hand-compact 同型:`PHALANX_2` 差 2,其餘 0。同一條連桿、同一個遠端零件在動,
  預算 `2` 由第二個實例獨立確認一次。
- `--real` 30 條全綠。
- **使用者 2026-09-09 Lane B 通過三張圖,`models/hand-gripper/` 已發布。**
  位元組逐項對照的包測試就位;README 的件數詞、尺寸三元組、對指距離與靜止淨距
  都由網格與規格反推驗證,不是散文。真機差分閘門加入後 `--real` **31 條全綠**。
