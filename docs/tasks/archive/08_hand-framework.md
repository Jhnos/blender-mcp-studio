...
- V01.0Q.002:`scripts/` 納入行數棘輪,真機驗證器與中空側鉸鏈產生器沿責任切開;17 份契約仍全過。
- V01.0Q.003:試片七項改由 `analyse_coupon.py` 判(使用者只量);精簡實例的發布路徑用合成檔先證明。

### Open failures

- 沒有機器檢查在失敗。
- 本任務機器側全部完成,剩下的每一步都在使用者手上;核心側的收債另開 `09_core-boundaries.md` 進行中
