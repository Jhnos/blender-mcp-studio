...
**`Verified-by` 回答的是「怎麼驗」,不是「驗過了沒有」。** 一個解析得到的 ref 只代表
那個檢查**存在**;它跑了沒有、母數多少,寫在 [[hand-v3/v7-matrix]] 的狀態欄。
台架那幾列的檢查都已經寫好,而 `analyse_bench_results.py` 現在對它們一律回報
`vacuous — n=0`——**那是檢查在運作並如實說沒有資料**,不是檢查還沒建。
...
|---|---|---|---|---|---|
| VOC-1 | 使用者看得到「充氣後抓得更牢」的實測數字,含母數、效果量與信賴區間 | voc | yes | feature | artifact:test_every_reported_scenario_carries_its_population_and_an_interval; differential:test_trials_pair_by_object_across_the_two_conditions; unit:test_a_complete_scenario_reports_magnitude_uncertainty_and_a_verdict |
...
| PS-6 | 外層改用不可延展手套時,夾層壓力才建得起來 | product | yes | flag | differential:test_the_glove_comparison_names_two_distinct_outer_layers; artifact:test_the_glove_comparison_names_two_distinct_outer_layers |
| PS-7 | 加壓與抽真空共用同一組介面,兩種都能跑 | product | yes | flag | differential:scenario_pairs; artifact:channel_probes |
