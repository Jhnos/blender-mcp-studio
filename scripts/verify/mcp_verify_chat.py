...
"""
import socket, json, ssl, time, random, urllib.request, sys
...
def oracle(code, timeout=10):
    s = socket.create_connection(ORACLE, timeout=timeout); s.settimeout(timeout)
...
def main():
    results = []
    def rec(hid, ok, detail):
        results.append((hid, ok, detail)); print(f"[{'PASS' if ok else 'FAIL'}] {hid}: {detail}")

    print("=== PRECHECK ===")
    base_names = o_names(); base_n = len(base_names)
    ...
    try:
        content, bout, sid = ws_chat(cmd)
        print(f"  reply: {content[:160]!r}")
        print(f"  blender_output: {str(bout)[:160]!r}")
    except Exception as e:
        rec("H0", False, f"WS chat failed: {e}"); content = bout = sid = None

    time.sleep(1.5)
    after_names = o_names()
    exists = nonce in after_names
    rec("H0", exists, f"oracle sees {nonce}? {exists}. objects now={len(after_names)} {after_names}")

    if exists:
        print(f"\n=== H4: differential count ===")
        rec("H4", len(after_names) == base_n + 1,
        ...
        try:
            ...
            fe_names = [o["name"] for o in scene.get("objects", [])]
            rec("H6", nonce in fe_names,
