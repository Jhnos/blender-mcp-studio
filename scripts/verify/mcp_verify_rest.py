...
"""
import socket, json, ssl, time, random, urllib.request

TS = "bearmacminimac-mini.tail56c751.ts.net"
BASE = f"https://{TS}/blender"

def oracle(code, timeout=10):
    ...
    while True:
        ...
        if not c: break
        buf += c
        try: r = json.loads(buf.decode()); s.close(); return r
        except json.JSONDecodeError: continue
    s.close(); return json.loads(buf.decode()) if buf else None

def o_out(code):
    r = oracle(code)
    return r["result"]["result"].strip() if r and r.get("status") == "success" else f"<err:{json.dumps(r)[:150]}>"

def o_names():
    return json.loads(o_out("import bpy,json\nprint(json.dumps([o.name for o in bpy.data.objects]))"))
...
def main():
    R = []
    def rec(h, ok, d): R.append((h, ok, d)); print(f"[{'PASS' if ok else 'FAIL'}] {h}: {d}")

    teardown()
    base = o_names(); base_n = len(base)
    ...
    rec("H2-geometry", verts == "8", f"{n} real vertex count = {verts} (cube=8; only real bpy yields this)")

    # H6: frontend REST /api/scene reflects real Blender (independent read path)
    st, scene = rest("GET", "/api/scene")
    fe = [o["name"] for o in scene.get("objects", [])]
    ...
    time.sleep(0.8)
    names = o_names()
    rec("H-rename", (n2 in names and n not in names), f"REST rename {n}->{n2}; oracle names={names}")
    ...
    rec("H4-delete", len(o_names()) == base_n, f"count back to baseline {base_n}? now {len(o_names())}")

    teardown()
    print(f"\nfinal oracle: {o_names()}")
    print("\n=== EVIDENCE MATRIX ===")
    for h, ok, d in R: print(f"  {h:22} {'PASS' if ok else 'FAIL'}")
