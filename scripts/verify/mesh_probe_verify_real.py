...
for x, y, r in config["holes_mm"]:
    ...
    for px, py in samples:
        hits.append(bool(body.ray_cast(Vector((px/1000, py/1000, -.1)), Vector((0,0,1)))[0]))
slopes = []
for origin in config["surface_origins_mm"]:
    hit, point, normal, face = body.ray_cast(Vector(tuple(v/1000 for v in origin)), Vector((0,0,-1)))
    slopes.append(math.degrees(math.acos(min(1, abs(normal.z)))) if hit else None)
...
def world_tree(obj):
    ...
    try:
        ...
        bmesh.ops.transform(mesh, matrix=obj.matrix_world, verts=mesh.verts)
