...
### V01.0T.001 — 可編輯來源操作說明

- 補齊單角色渲染時的集合隔離、四角色組裝取景與還原設定；校正生成器說明中的舊數量。
- 文件及註解修訂的T1/T2閘門通過；幾何及blend未改，沿用V01.0T.000完整真Blender證據。

### V01.0T.000 — 四角色與六類事件標記

- 擴充守衛與工匠，四角色共192幀、四張完整臉部頭像、18事件標記；保留既有影格索引。
- 發布器新增組裝預覽透明邊界檢查，防止角色增加後正交相機裁切。
- 真Blender重開、骨架／錨點／動作差分與完整CI通過；可玩送件驗證由PVR追蹤。

### V01.0S.001 — 人物素材驗證交接

- 將本切片真 Blender 閘門、獨立視覺複驗與後續角色範圍收斂至標準任務交接；保留其他分支任務的歸屬。

### V01.0S.000 — Living Actors 首個可玩人物切片

- 新增旅人／嚮導共用骨架、命名動作、四方向96張sprite與兩張頭像；沿用已交付正交相機。
- 新增九個可編輯事件徽章與窄發布器；來源重開、動作差分、根節點錨點及臉部取景驗證。
- 頭像取景的獨立視覺發現已加入幾何邊界oracle；原有room整合缺口仍分開追蹤。


### V01.0R.000 — World Kit 多面向素材

- 新增18種建築、戶外、生活與機關模型；兩材質、四雙態、44sprites、六組組裝場景。
- 窄介面發布器輸出既有格式圖集、Tiled配置與可下載Blender模型包；實際原點投影及接點檢查進真Blender CI。

### V01.0Q.001 — 通用素材驗收責任更正

- 可量化視覺與操作驗證留在 Lane A；移除要求使用者代驗的交接，列明正式整合與證據缺口。

### V01.0Q.000 — Veilroom 通用遊戲素材

#### Added

- 12 種公尺制低多邊形模組、木石與金屬兩材質、門／箱子開關態與側牆朝向。
- 可編輯素材庫、組裝倉庫、正交 PNG、圖集發布器與獨立 Blender 重開驗證。
- Veilroom 地圖快照與使用說明；模型保留於本專案，遊戲靜態素材發布至 Veilroom。
