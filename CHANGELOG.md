...
- 對照 GOnDO 原廠實物照片收小臂端盤、梁寬、雙桿間距與三角平台，保留軸孔／臂厚及兩頭共杯定位；不增加零件或五金數。
- 更新閉合尺寸測試，46 個位置／16 折屏角度與實網格斷接负對照通過；完整 real CI 全綠。列印被動軸防脫、旋鈕齒槽與負載資格仍未完成。

### V01.0R.01J — 分段電極臂與三角平台概念

#### Added

- 從簡化雙節臂延伸前臂平行雙桿與三角探頭平台，肩與前臂共同定位，保留兩頭獨立及手調探頭角度。依 Agilent／ELMETRON／GOnDO 實驗室電極臂整理參考。
- 46 個單頭取樣、16 個折屏角度、結構件全配對自碰與共同孔軸、移走連桿負對照；沿用 real CI 入口。調整機構 12 螺絲／12 螺母及 4 列印被動軸包絡，防脫、齒槽及負載未定稿。

### V01.0R.01I — 簡化雙節臂概念與連接驗證

#### Added

- 依使用者修正改為兩支獨立雙節臂與整合上蓋的螢幕支承，調整機構 10 螺絲／10 螺母；取消主方案四連桿、軸套與獨立墊片。新增空間／孔材料檢查與斷接負對照，仍為概念而非製造版。
- 以下長穿軸修復留在舊 rotary 回歸中，不再擴充為主方案。

- 兩側旋轉底座新增軸頸、名義金屬軸套、M5 穿軸與上下保持件，支承柱連到底板／側壁，底腳避讓螺栓頭。
- 共同通孔、五金間隙與每側五個止擋位移檢查；移開上墊片的負對照，以及獨立剖面副本圖。
- 泵浦佔位向內移 10 mm 避開支柱。五金選型、承力、裝入工具與新機箱匯出資格仍未完成。

### V01.0R.01H — 修復手臂斷接與腕部介面

#### Fixed

- 修復薄梁肩部接頸與齒盤間斷接；前臂接回段整合為一體，移除沒有接頭的獨立末段。
- 新增腕部叉耳／舌片／穿軸，調整接回路徑避開肘部與腕部轉動；結構網格守衛擴到 14 件。

#### Added

- 肩部脫齒與獨立控制；腕部三角度的共同開孔、周圍實體與五金間隙檢查，以及移開叉耳的斷接負對照。底座固定到機箱的介面仍未完成，非整機連接或製造資格。

### V01.0R.01G — rotary 支撐匯出與獨立契約

#### Added

- 四件肘部支撐毫米 STL、分離檢查副本與左右 readiness 契約；禁止非流形、法線、退化、自交與分析截斷。
- 獨立讀取二進位 STL，與 Blender 世界頂點尺寸比對，容差 0.02 mm。僅為支撐試配匯出，未發布整機製造包。

#### Fixed

- 匯出副本位移後先更新再隱藏，修復第一次 oracle 讀到舊世界座標的錯誤。

### V01.0R.01F — 肘部工具入口

#### Fixed

- rotary 肘部螺栓增加內六角孔，修復扳手直段無法就位。

#### Added

- 收折 HMI 後兩側六角扳手／套筒四個就位檢查，以及封孔必須拒絕的負對照。僅驗名義直段包絡，未宣稱手柄擺幅或施力資格。

### V01.0R.01E — 肘部支撐網格與装入路徑

#### Fixed

- 固定接頸改用實心梁，修復四件 rotary 支撐的非流形邊與退化面；保留齒槽與升降碰撞檢查。

#### Added

- 四件單連通閉合網格守衛與開面負對照。
- 沿用肘部裝入檢查，驗證先收折 HMI 後的 216 個五金取樣位置；工具擺幅、自交與承重尚未取得資格。

### V01.0R.01D — 四連桿支撐肘部齒槽

#### Added

- rotary 支撐肘部改為整合齒盤的兩個成員，沿用既有名義穿軸與墊片；加入 2 mm 脫齒控制、兩側五狀態檢查及局部圖。
- 脫齒位移納入獨立控制驗證。肩、腕與底座承力仍未取得製造資格。

#### Fixed

- 下臂接回路徑繞過齒盤與四連桿，修復中立姿態及升降時的表面干涉。

### V01.0R.01C — 四連桿五金與獨立關節控制

#### Added

- 每頭恢復底座、肩、肘、腕四個旋轉控制與升降，10 項真機隔離檢查及停用驅動負對照。支撐仍為概念包絡，非製造資格。
- 名義 M5 轉軸、三墊片及兩段軸套；雙頭 121 組高度配對、同頭移動件與 144 組折屏檢查，加入跨頭侵入負對照。

#### Fixed

- 新控制完成相依關係後重編譯驅動，修復欄位存在但零件不動；真機入口重新載入既有模組，避免沿用舊版記憶體程式。

### V01.0R.01B — 獨立四連桿升降概念

#### Added

- 回應使用者以活動臂取代線性滑座的方向：兩組 130 mm 平行桿、100 mm 升程，實際桿端閉合與獨立頭部檢查；新增可開啟模型與三種姿態。
- 真 Blender 202 個探頭路徑取樣、42 個整機姿態與斷開連桿負對照接入本地 CI。僅為運動概念，不宣稱製造／承載合格。
- V10 基線肩部齒盤接合、脫齒控制及緊湊五金；支座到機箱承力未完成。

#### Fixed

- 修復孔壁共面非流形與肩部五金擋住折平螢幕；夾具場景以必要零件名稱核對，取代可被無關物件湊數的總量判斷。

### V01.0R.01A — 肘部墊片與装入順序

#### Added

- 肘部雙側墊片，216 個裝入取樣與脫齒時的五金干涉檢查。
- 螺栓穿出螺母的軸向餘量守衛及不足餘量負對照；仍非螺紋／預緊／保持力資格。

### V01.0R.019 — 肘端一體齒盤與脫齒控制

#### Added

- 上下臂側向接座與一體齒盤、通軸孔及名義五金；新增上臂 STL，共 27 件試配，手臂獨立 readiness 分批契約。
- 肘部 2 mm 脫齒控制、兩側五狀態負對照與五金干涉檢查，控制隔離擴為 12 案例。

#### Fixed

- 參考肘端臂身／齒面／軸穿插；先穿孔再合併齒盤，並在手臂收尾處理三角細碎面與極近重複點，保持原 readiness 判準。
- 螺栓若隨下臂脫齒會撞上臂，改由上臂持有螺栓、下臂持有螺母。承重及完整裝配仍未取得資格。

### V01.0R.018 — 腕軸工具直段包絡

#### Added

- 腕軸加入六角孔與扳手／套筒直段包絡檢查，示意圖與驗證共用相同幾何；沒有把假設工具尺寸視為商品選型或力矩資格。
- V7 輸出獨立目錄，保留前版試配。

### V01.0R.017 — 腕部叉耳與裝入路徑試配

#### Added

- 腕部中間轉動件與背架合一、叉耳與下臂合一，新增兩支試配 STL；名義 M5 軸件、螺母與兩側墊片。
- 左右腕部共 22 個局部轉動姿態、126 個裝入位置檢查；保持力、防鬆、工具空間及肘端連接仍未取得資格。

#### Fixed

- 腕部参考圓柱與背架／下臂穿插；腕軸後退並改為上方橋接，保留原有螢幕收折範圍。
- 叉耳接合共面導致非流形，改用有重疊且不共面的布林接合；全部 25 件試配各為單一封閉實體。

### V01.0R.016 — 滑座鎖緊五金與接觸負對照

#### Added

- 每頭補入 M4×12 名義平端手轉螺絲與螺母包絡，鬆開狀態保留 0.5 mm 導桿間隙；真模型驗證缺件、旋入途中卡阻及過行程撞桿負對照。
- V5 輸出獨立目錄；保持力仍未取得資格，不以幾何接觸代替承重證據。

### V01.0R.015 — 探頭可拆軟夾與導桿防脫試配

#### Added

- 兩件夾蓋、六件半襯、兩件導桿壓蓋及標準五金參考，23 件試配 STL；螺栓装入、上下止擋與 126 個拆夾操作取樣。尚未通過實體保持力與全機承力資格。
- 探頭五金納入螢幕收折檢查，新增選取母數負對照；夾具 readiness 分批契約保持禁止截斷。

#### Fixed

- 擴大夾爪後背架擋住升降，調整背架間隙；右夾頭內側裝螺栓會穿過左探頭，改由外側裝入。
- 共用 artifact oracle 的數字尾碼排序混用整數與字串，改成一致的具型別排序鍵並加入回歸測試。

### V01.0R.014 — 工作站探頭改成 100 mm 直上抽出，建立操作驗證

#### Added

- 量測工作站腳本模型：共杯雙臂、Waveshare 5 吋外殼、24 齒關節試片、可折平螢幕、兩組導桿／升降滑座；13 件試配 STL，尚非全機製造發布。
- VOC／BDD 情境／DFMEA 驗證計畫；兩頭清杯高度、固定件隔離、直線保守掃掠、提起後轉開與螢幕姿態的真網格證據及圖片。
- 外殼與升降件分批 readiness contract 接入 `ci.sh --real`，均拒絕截斷取樣；純領域與幾何、姿態、驗證、渲染分離。

#### Fixed

- V2 的 35 mm 微調不足以提離杯口。腕關節後退 40 mm，探頭改由 100 mm 獨立直線滑座提升；驗證不再以「兩頭互不連動」替代清杯操作。


### V01.0R.013 — 場景清單不再只有十個;對話路徑第一次有真機閘門

#### Fixed

- `scene_summary()` 的物件清單改由 adapter 有上界地讀,不再吃 addon 那份截斷到十筆的摘要。
  上界 500(實測最大場景 209),碰到上界由 `objects_truncated` 說出來——把靜默的 10
  換成靜默的 500,是同一個缺陷晚一個數量級。位置解碼共用 `decode_vector`,不留第二份。

#### Added

- `scripts/verify/mcp_verify_chat.py` 進 `ci.sh --real`(H6 綠了才接),由架構測試釘住。
  **對話路徑第一次有真機閘門**;`--real` 32 條全綠。

### V01.0R.012 — 任務 13 使用者驗收歸檔

### V01.0R.011 — 對話能呼叫本專案自己的產生器

#### Added

- 對話工具清單新增 `list_instances` 與 `build_instance`。在此之前,對話能產出的極限是
  方塊、曲線、燈、相機——**沒有一句話能做出可印的零件**,因為唯一會產生可印幾何的東西
  (參數化產生器)對話看不到。上一輪為 REST 補的那條落差,在對話這層原封不動存在一次。
- `src/core/use_cases/conversation_generation.py`:工具定義與執行,從 `conversational_modeling`
  拆出來(後者撞到 400 行預算;專案規則是拆檔不是壓縮語意)。
- 守衛:產生類工具**在語意路由之後**才附加(路由為省 token 丟工具,丟掉唯一能做可印零件的
  那一個等於能力靜默消失);slug 選項 == 登錄表;`tests/e2e` 一條盯著公開九項目錄不與
  對話工具清單合流。

#### Notes

- 任務 13 的規格自相矛盾:同時要求「把對話驗證器接進 `ci.sh`」和「H6 不修」,
  但那支驗證器因 H6 而 exit 1。**能力照做,閘門不加**,連同 H6 移到任務 15。
  明知會紅的閘門進 CI,只會讓人學會忽略它。
- H6 根因查明:上游 addon `__init__.py:283` 明文 `limit to first 10 objects`。

### V01.0R.010 — 對話路徑的驗證器連 import 都過不了

#### Fixed

- `scripts/verify/mcp_verify_chat.py` 缺 `sys.path` 注入,第一行實質工作就 `ModuleNotFoundError`。
  沒人發現,因為**它不在 `ci.sh` 裡**——沒被引用的腳本和壞掉的腳本從外面看一模一樣。

#### Notes

修好後首跑 **4/5**:H0(獨立 oracle 看得到)、H2(真實頂點數)、H4(差分計數)、H7(REST 刪除後確實消失)過;
**H6 失敗**——`/api/scene` 只回 10 個物件,而場景有 209 個,所以前端場景清單在大場景下
結構上就反映不了真實狀態。上限來自上游 addon 的 `get_scene_info`(ADR-001 把它整包包住),
本專案的程式碼裡沒有這個截斷。**尚未修,先記錄。**

### V01.0R.00F — 試片判讀對無軸承連桿是錯的,修好

在準備列印工作單時發現兩個會直接誤導動手的人的缺陷。兩個都是同一族:
**量測旁邊的散文沒有跟著規格走。**

#### Fixed

- `coupon_judgement` 從來沒查過 `link.has_bearing_seat`。compact 連桿(人手那件與夾爪都用它)
  無軸承座,C3/C4 卻仍被要求量,於是那兩個包的試片**永遠不可能通過**——一道不可能通過的檢查
  只會教人忽略結果。改判 `N/A`,並要求至少一項適用才算通過(全部 N/A 而報 PASS 正是這個
  模組本來就拒絕的空洞為真)。無軸承連桿的公差帶也從兩條減為一條。
- 是非題的文字寫死 V3 的 Ø4.0 銷與 Ø2.0 探針線。compact 連桿的銷是 2 mm、腱孔 1.5 mm——
  照舊文字做的人會拿比孔粗的線去穿,然後記下一個零件根本沒有的失敗。文字改由連桿推導。

兩者都附反向測試:有軸承座的連桿不得因此變綠,探針線不得再是字面值。

### V01.0R.00E — 任務 12 驗收歸檔;新增一條教訓

#### Added

- `docs/LESSONS_LEARNED.md`:「為擋外來輸入設計的安全閘,會在自家工具搬上產品路徑那天擋住自己」。
  預防法是把豁免鍵在**可重新計算的性質**上而非呼叫者身上,豁免與消費端共用同一個推導函式,並附反向測試。

#### Changed

- 使用者 2026-09-09「12 可以歸檔」:`docs/tasks/12_hand-gripper.md` → `archive/`。
  三步順序走完,現在沒有進行中的任務;四件包都已發布,四件都沒印過。
